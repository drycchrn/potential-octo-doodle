...
function vowelCount(string) {
  ...
  for (let letter of string) {
    let currentLowerCaseLetter = letter.toLowerCase();

    if (vowels.includes(currentLowerCaseLetter)) {
      if (vowelMap.has(currentLowerCaseLetter)) {
        vowelMap.set(currentLowerCaseLetter, vowelMap.get(currentLowerCaseLetter) + 1);
      } else {
        vowelMap.set(currentLowerCaseLetter, 1);
      }
    }
  }
